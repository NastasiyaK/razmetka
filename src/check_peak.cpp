...
bool one_lead::check_peak()
{
	...
	if (array_of_peak_R.size() !=0 &&
	(new_peak - *(array_of_peak_R.end()-1) > 0.8 * (length.T_middle + length.ST_seg + QRS.height/2) * Fs || (array_of_peak_R.size() <= 2  )) &&
    (diff > 0.35 * average_amplitude.get()|| filter_signal.at(dist) >  2 * QRS_hight_min) &&
			(filter_signal.at(dist) > 0.8 * QRS_filtered_min && diff > min(0.35f * diff_last_peak,0.45f))
			
			)
	{
		
		int dist2 = set_indices(*(array_of_peak_R.end() - 1), count_iter, mem, mem_sdvig);
		
		if (dist2 + static_cast<int>( (Fs * QRS.height) /2) < signal.size() + 1)
		
		if ( signal.size() - dist2 < static_cast<size_t>(Fs*QRS.low)/2)
		{
			if (*max_element(filter_signal.begin() + dist2, filter_signal.begin() + dist2 + static_cast<size_t>(Fs * QRS.low) / 2) >
			    filter_signal.at(dist2))
			{
				array_of_peak_R.erase(array_of_peak_R.end() - 1);
				peaks_with_types.erase(peaks_with_types.end() - 1);
			}
		...
		if (!array_of_peak_R.empty() && ((array_of_peak_R.size() == 1 ) ||
			(array_of_peak_R.size() >= 2 && (new_peak - *(array_of_peak_R.end()-1) > 0.8 * (length.T_middle + length.ST_seg + QRS.height/2) * Fs ) )))
			{
			...
			{
				int peak1 = set_indices(*(array_of_peak_R.end() - 1), count_iter, mem, mem_sdvig);
				if (filter_signal.at(peak1) < 0.5 * filter_signal.at(dist))
				{
					QRS_hight_min = amplitude_new_peak / 3;
					*(array_of_peak_R.end() - 1) = new_peak;
	...
	if (array_of_peak_R.size() !=0 && new_peak - *(array_of_peak_R.end()-1) <= 2 * Fs * QRS.low &&
	...
	if (array_of_peak_R.size() > 1 && *(array_of_peak_R.end()-1) - *(array_of_peak_R.end()-2) <= 1.1 * (length.T_middle + length.ST_seg + QRS.height/2) * Fs
			&& new_peak - *(array_of_peak_R.end()-1) <  0.8 * (length.T_middle + length.ST_seg + QRS.height/2) * Fs && diff > QRS_filtered_min * 0.8)
	{
		*(array_of_peak_R.end() - 1) = new_peak; //there's normal R beat
		(peaks_with_types.end() - 1)->first = new_peak;
	}
