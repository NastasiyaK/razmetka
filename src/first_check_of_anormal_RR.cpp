...

{
	...
	if (array_of_extrasys.empty())
	{
		...
		if (array_of_peak_R.size() >= 2 && *(array_of_peak_R.end() - 1) - *(array_of_peak_R.end() - 2) >  max(average_R*1.4f, static_cast<const float>(RR.middle*Fs)))
		{
			len_R = *(array_of_peak_R.end() - 1) - *(array_of_peak_R.end() - 2);
			int i2 = *(array_of_peak_R.end() - 1) - static_cast<int>(QRS.height*Fs);
			int ind_vn;
	...
	if (array_of_extrasys.size() == 1 &&  abs(array_of_extrasys.at(0) - new_peak) > Fs*RR.middle / 4)
	{
		...
		if (((float)(dist) / (dist2) > 1.2 || ((float)(dist) / (dist2)) < 0.8) && ind_extra > 0)
		{
			...
			float diff_end_of_peak = filter_signal.at(ind_extra) - filter_signal.at(ind_extra + static_cast<int>( Fs*QRS.height ));
			...
			if (((float)(dist) / (dist2) < 1.2 && ((float)(dist) / (dist2)) > 0.8))
			{
				...
				float dif_stop_R = signal.at(index_of_extrasys) - signal.at(index_of_extrasys + static_cast<int>(Fs*(QRS.height) / 2));
				float dif_start_R = signal.at(index_of_extrasys) - signal.at(index_of_extrasys - static_cast<int>(Fs*(QRS.height) / 2));
